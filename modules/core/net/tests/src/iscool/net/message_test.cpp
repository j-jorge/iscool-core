...
TEST( message_test, serialization )
{
    ...
    const iscool::net::message serialized
        ( iscool::net::deserialize_message
    ...
    const iscool::net::byte_array serialized_content
...
TEST( message_test, content_header )
{
    ...
    EXPECT_EQ
        ( reference.get_type(), reader.get< iscool::net::message_type >() );
    EXPECT_EQ( expected_size, reader.get< std::uint16_t >() );
    EXPECT_EQ
        ( reference.get_session_id(), reader.get< iscool::net::session_id >() );
    EXPECT_EQ
        ( reference.get_channel_id(), reader.get< iscool::net::channel_id >() );
