...
static void expect_encode_base64_eq
( const std::string& output, const std::string& input_string )
{
    const iscool::net::byte_array input
        ( iscool::net::tests::std_string_to_byte_array( input_string ) );
    EXPECT_EQ
        ( output, iscool::net::encode_base64_string( input ) );
}

TEST( encode_base64_string, empty )
{
    EXPECT_EQ
        ( "", iscool::net::encode_base64_string( iscool::net::byte_array() ) );
}
