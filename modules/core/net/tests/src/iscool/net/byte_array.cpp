...
TEST( byte_array_test, begin_end_order )
{
    ...
    EXPECT_EQ
        ( bytes, std::vector< std::uint8_t >( array.begin(), array.end() ) );
}
...
TEST( byte_array_test, constructor_from_range_large )
{
    ...
    EXPECT_EQ( expected,
               std::vector< std::uint8_t >( array.begin(), array.end() ) );
}
...
TEST( byte_array_test, append_non_empty_to_non_empty )
{
    ...
    const iscool::net::byte_array inserted
        ( inserted_bytes.begin(), inserted_bytes.end() );
