...
namespace iscool
{
    namespace net
    {
        namespace detail
        {
            ...
            static byte_array extract_byte_array
...
iscool::net::decode_base64_string( const std::string& string )
{
    ...
    for( auto it( string.begin() );
         it != string.end() && detail::is_base64( *it ); ++ it )
    {
    ...
    if( cursor != character.begin() )
    {
        ...
        result.append
            ( byte_array_reader
              ( to_append ).slice( cursor - character.begin() - 1 ) );
    }
...
iscool::net::byte_array iscool::net::detail::extract_byte_array
( std::array< std::uint8_t, 4 > character )
{
    ...
    resulting_array[ 1 ] =
        ...
        ( ( character[ 2 ] & 0x3c ) >> 2 );
    resulting_array[2] =
        ( ( character[ 2 ] & 0x3 ) << 6 ) | character[ 3 ];

    return iscool::net::byte_array
        ( resulting_array.begin(), resulting_array.end() );
}
