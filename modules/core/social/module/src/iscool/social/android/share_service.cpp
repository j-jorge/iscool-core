...
( const std::string& path )
{
    ...
    return
        std::strncmp
        ( path.c_str(), assets_prefix, std::strlen( assets_prefix ) )
        == 0;
}

std::string iscool::social::android::detail::copy_asset
( const std::string& path )
{
    ...
    const std::string result
        ( iscool::files::get_writable_path() + "/shared-asset" + extension );
    ...
    {
        ic_causeless_log
            ( iscool::log::nature::error(), "iscool::social::share_file",
