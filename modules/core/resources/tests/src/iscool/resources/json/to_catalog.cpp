...
TEST(iscool_resources_catalog_json, to_catalog)
{
  ...
  const iscool::resources::catalog catalog(
      iscool::resources::json::to_catalog(iscool::json::parse_string(json)));

  const std::vector<std::string> textures{ "texture.png", "texture1.jpg",
                                           "texture3.png" };
  ...
  const std::vector<std::string> sounds{ "sound_1", "sound_2", "sound_9" };
  iscool::test::assert_equal_collections(sounds, catalog.get_sounds());

  const std::vector<std::string> styles{ "style.json", "style-b.json" };
  iscool::test::assert_equal_collections(styles, catalog.get_styles());

  const std::vector<std::string> skeletons{ "texture" };

  iscool::test::assert_equal_collections(skeletons, catalog.get_skeletons());

  const std::vector<std::string> sprite_sheets{ "texture1.plist",
                                                "texture3.plist" };

  iscool::test::assert_equal_collections(sprite_sheets,
...
TEST(iscool_resources_catalog_json, to_catalog_fail_general)
{
  ...
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::Value()),
               iscool::json::bad_cast);
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::intValue),
               iscool::json::bad_cast);
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::uintValue),
               iscool::json::bad_cast);
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::arrayValue),
               iscool::json::bad_cast);
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::stringValue),
               iscool::json::bad_cast);
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::realValue),
               iscool::json::bad_cast);
  EXPECT_THROW(iscool::resources::json::to_catalog(Json::booleanValue),
               iscool::json::bad_cast);
