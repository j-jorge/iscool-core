...
( const std::vector< std::string >& textures,
  ...
  const std::vector< std::string >& styles )
    : _textures( textures ),
      ...
      _styles( styles )
{
    assert( !iscool::collections::has_duplicates
    ...
    assert( !iscool::collections::has_duplicates
            ( _skeletons.begin(), _skeletons.end() ) );
    assert( !iscool::collections::has_duplicates
            ( _sprite_sheets.begin(), _sprite_sheets.end() ) );
}
...
{
    _textures.insert
        ( _textures.end(), that._textures.begin(), that._textures.end() );
    assert
        ( !iscool::collections::has_duplicates
          ( _textures.begin(), _textures.end() ) );
    
    _skeletons.insert
        ( _skeletons.end(), that._skeletons.begin(), that._skeletons.end() );
    assert
        ( !iscool::collections::has_duplicates
          ( _skeletons.begin(), _skeletons.end() ) );
    
    _particles.insert
        ( _particles.end(), that._particles.begin(), that._particles.end() );
    assert
        ( !iscool::collections::has_duplicates
          ( _particles.begin(), _particles.end() ) );
    
    _sprite_sheets.insert
        ( _sprite_sheets.end(), that._sprite_sheets.begin(),
    ...
    _sounds.insert
        ( _sounds.end(), that._sounds.begin(), that._sounds.end() );
    assert
        ( !iscool::collections::has_duplicates
          ( _sounds.begin(), _sounds.end() ) );
    
    _styles.insert
        ( _styles.end(), that._styles.begin(), that._styles.end() );
    assert
        ( !iscool::collections::has_duplicates
...
( const std::string& filename )
{
    std::string::size_type i
        ( detail::check_extension( ".png", filename ) );
