...
iscool::jni::bad_cast::bad_cast
( jobject value, const std::string& expected )
{
    std::string cpp_string;

    if( value != nullptr )
    {
        const auto jobject_to_string
            ( jni::get_method< jobject >
    ...
    _message =
        "JNI value is not a valid " + expected + ": "
        + cpp_string;
}
