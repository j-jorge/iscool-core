...
void iscool::jni::throw_java_exception
( const std::string& type, const std::string& what )
{
    const java_ptr< jclass > class_id( detail::get_class( type.c_str() ) );
...
{
    const java_ptr< jclass > class_id( detail::get_class( "java/lang/Error" ) );
    detail::get_jni_env()->ThrowNew( class_id.get(), what.c_str() );
}
