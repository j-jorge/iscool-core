...
{
    Json::Value value;
    EXPECT_THROW( iscool::json::cast< float >( value ),
                  iscool::json::bad_cast );

    Json::Value value0( "10" );
    EXPECT_THROW( iscool::json::cast< float >( value0 ),
                  iscool::json::bad_cast );

    Json::Value value1( true );
    EXPECT_THROW( iscool::json::cast< float >( value1 ),
                  iscool::json::bad_cast );

    Json::Value value2( Json::arrayValue );
    EXPECT_THROW( iscool::json::cast< float >( value2 ),
                  iscool::json::bad_cast );

    Json::Value value3( Json::objectValue );
    EXPECT_THROW( iscool::json::cast< float >( value3 ),
                  iscool::json::bad_cast );
}
...
TEST( iscool_json_cast, to_float_fallback )
{
    ...
    EXPECT_FLOAT_EQ
        ( 1, iscool::json::cast< float >( Json::Value( 1.f ), 8.f ) );
}
