...
TEST( iscool_json_cast, to_unordered_map_throw )
{
    ...
    Json::Value value;
    EXPECT_THROW( iscool::json::cast< map_t >( value ),
                  iscool::json::bad_cast );

    Json::Value value0( "10" );
    EXPECT_THROW( iscool::json::cast< map_t >( value0 ),
                  iscool::json::bad_cast );

    Json::Value value1( std::uint64_t( 0x123456789abcdef ) );
    EXPECT_THROW( iscool::json::cast< map_t >( value1 ),
                  iscool::json::bad_cast );

    Json::Value value2( Json::objectValue );
    EXPECT_THROW( iscool::json::cast< map_t >( value2 ),
                  iscool::json::bad_cast );

    Json::Value value3( 5.4f );
    EXPECT_THROW( iscool::json::cast< map_t >( value3 ),
                  iscool::json::bad_cast );

    Json::Value value4( -10 );
    EXPECT_THROW( iscool::json::cast< map_t >( value4 ),
                  iscool::json::bad_cast );

    Json::Value value5;
    value5[ 0 ][ "key" ] = 1.5;
    value5[ 0 ][ "value" ] = "abc";
    EXPECT_THROW( iscool::json::cast< map_t >( value5 ),
                  iscool::json::bad_cast );

    Json::Value value6;
    value6[ 0 ][ "eky" ] = 1;
    value6[ 0 ][ "avlue" ] = "abc";
    EXPECT_THROW( iscool::json::cast< map_t >( value6 ),
                  iscool::json::bad_cast );
}
...
TEST( iscool_json_cast, to_unordered_map_fallback )
{
    ...
    EXPECT_EQ
        ( expected_1,
          iscool::json::cast< map_t >( Json::Value(), expected_1 ) );

    const map_t expected_2( { { "18", 90 }, { "fourty-four", 44 } } );
    EXPECT_EQ
        ( expected_2,
          iscool::json::cast< map_t >( Json::Value(), expected_2 ) );
