...
#include <iscool/json/is_of_type_uint8.hpp>
...
TEST(iscool_json_cast, to_uint8)
{
  ...
  EXPECT_EQ(std::uint8_t(0), iscool::json::cast<std::uint8_t>(value0));

  const Json::Value value2(100);
  EXPECT_EQ(std::uint8_t(100), iscool::json::cast<std::uint8_t>(value2));

  const Json::Value value5(std::uint8_t(255));
  EXPECT_EQ(std::uint8_t(255), iscool::json::cast<std::uint8_t>(value5));
}
...
{
  const Json::Value value;
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value),
               iscool::json::bad_cast);

  const Json::Value value0("10");
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value0),
               iscool::json::bad_cast);

  const Json::Value value1(true);
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value1),
               iscool::json::bad_cast);

  const Json::Value value2(Json::arrayValue);
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value2),
               iscool::json::bad_cast);

  const Json::Value value3(Json::objectValue);
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value3),
               iscool::json::bad_cast);

  const Json::Value value4(-100);
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value4),
               iscool::json::bad_cast);

  const Json::Value value5(-123456789);
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value5),
               iscool::json::bad_cast);

  const Json::Value value6(std::int64_t(-1234567890123456));
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value6),
               iscool::json::bad_cast);

  const Json::Value value7(5.4f);
  EXPECT_THROW(iscool::json::cast<std::uint8_t>(value7),
               iscool::json::bad_cast);
}
