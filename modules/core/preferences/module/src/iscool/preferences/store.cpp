...
iscool::preferences::store::store(const std::chrono::milliseconds& flush_delay,
...
void iscool::preferences::store::schedule_save()
{
  ...
  if (_flush_delay == std::chrono::milliseconds::zero())
    ...
    _save_connection = iscool::schedule::delayed_call(
...
template <typename T>
void iscool::preferences::detail::collect_keys::operator()(std::string key,
                                                           T&&)
{
