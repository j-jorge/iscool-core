...
#define ic_detail_context_field_type(seq) BOOST_PP_SEQ_ELEM(0, seq)

#define ic_detail_context_field_name(seq) BOOST_PP_SEQ_ELEM(1, seq)
...
#define ic_detail_context_is_method_name(seq)                                 \
  BOOST_PP_CAT(BOOST_PP_CAT(is_, ic_detail_context_field_name(seq)), _set)
...
#define ic_detail_context_rw_methods(r, data, seq)                            \
  ic_detail_context_get_rw_method(seq) ic_detail_context_set_method(seq)      \
      ic_detail_context_is_method(seq) ic_detail_context_reset_method(seq)
...
#define ic_detail_context_list_arguments(r, data, i, seq)                     \
  BOOST_PP_COMMA_IF(i) ic_detail_context_field_type(seq) BOOST_PP_CAT(arg, i)
...
#define ic_detail_context_set_fields_from_arguments(fields)                   \
  BOOST_PP_SEQ_FOR_EACH_I(ic_detail_context_set_field_from_argument, _, fields)
...
#define ic_detail_context_template_method_for_all(fields)                     \
  ...
  {                                                                           \
    context result(*this);                                                    \
    ic_detail_context_set_fields_from_arguments(fields) return result;        \
  }
...
#define ic_detail_context_default_constructor(fields) context() = default;

#define ic_detail_context_constructor(parent_fields)                          \
  BOOST_PP_IF(BOOST_PP_SEQ_SIZE(parent_fields),                               \
              ic_detail_context_template_constructor,                         \
              ic_detail_context_default_constructor)                          \
  (parent_fields)

#define ic_detail_context_template_constructor(parent_fields)                 \
  template <typename OtherType>                                               \
  context(const OtherType& other_context)                                     \
    : ic_detail_context_copy_fields_from_other(parent_fields, other_context)  \
  {}
