...
TEST(iscool_files, read_file)
{
  ...
  delegates.read_file_impl =
      [&file_name,
       &stream](const std::string& name) -> std::unique_ptr<std::istream>
  {
    file_name = name;
...
TEST(iscool_files, get_writable_path)
{
  ...
  delegates.get_writable_path_impl = []() -> std::string
  {
    return "the/path";
  };
...
TEST(iscool_files, get_full_path)
{
  ...
  delegates.get_full_path_impl = [&arg](const std::string& path) -> std::string
  {
    arg = path;
