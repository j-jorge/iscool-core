...
#define ic_detail_state_monitor_state_name(seq) BOOST_PP_SEQ_ELEM(0, seq)

#define ic_detail_state_monitor_successor_list(seq) BOOST_PP_SEQ_ELEM(1, seq)
...
#define ic_detail_state_monitor_states_enumeration(states)                    \
  ...
  {                                                                           \
    BOOST_PP_SEQ_FOR_EACH_I(ic_detail_state_monitor_state_value, _, states)   \
  };

#define ic_detail_state_monitor_is_method_name(name)                          \
  BOOST_PP_CAT(BOOST_PP_CAT(is_, name), _state())
...
#define ic_detail_state_monitor_is_method(r, data, seq)                       \
  ...
  {                                                                           \
    return _state == ic_detail_state_monitor_state_enum_value(seq);           \
  }

#define ic_detail_state_monitor_set_method_name(seq)                          \
  BOOST_PP_CAT(BOOST_PP_CAT(set_, ic_detail_state_monitor_state_name(seq)),   \
               _state())
...
#define ic_detail_state_monitor_is_successor_case_return(data, successors)    \
  BOOST_PP_IF(BOOST_PP_SEQ_SIZE(successors),                                  \
              BOOST_PP_SEQ_FOR_EACH_I(ic_detail_state_monitor_successor_test, \
                                      data, successors),                      \
              false)
...
#define ic_detail_state_monitor_is_current_state_successor(states)            \
  ...
  {                                                                           \
    switch (_state)                                                           \
      {                                                                       \
        BOOST_PP_SEQ_FOR_EACH(ic_detail_state_monitor_is_successor_case, s,   \
                              states)                                         \
      default:                                                                \
        assert(false);                                                        \
      };                                                                      \
...
#define ic_detail_state_monitor_state_to_string(states)                       \
  ...
  {                                                                           \
    switch (s)                                                                \
      {                                                                       \
        BOOST_PP_SEQ_FOR_EACH(ic_detail_state_monitor_to_string_case, _,      \
                              states)                                         \
      default:                                                                \
        return nullptr;                                                       \
      };                                                                      \
  ...
  private:                                                                    \
    ic_detail_state_monitor_states_enumeration(states)                        \
        ic_detail_state_monitor_is_current_state_successor(states)            \
            ic_detail_state_monitor_state_to_string(states) void set_state(   \
                state new_state)                                              \
    {                                                                         \
...
#define ic_implement_state_monitor(scope, field_name, initial_state, states)  \
  ic_detail_implement_state_monitor(scope::, field_name, initial_state, states)
...
#define ic_inline_state_monitor(field_name, initial_state, states)            \
  ...
  iscool::memory::pimpl<state_monitor_##field_name,                           \
