...
class iscool_audio_default_effect_player_test : public ::testing::Test
{
public:
...
TEST_F(iscool_audio_default_effect_player_test, play_effect)
{
  ...
  const iscool::audio::track_id b_id(
      iscool::audio::play_effect("b", iscool::audio::loop_mode::forever));
  EXPECT_TRUE(_implementation.effects.empty());
  EXPECT_TRUE(_implementation.effect_loops.empty());
...
{
  iscool::audio::play_effect("does-not-exist", iscool::audio::loop_mode::once);
  _mixer.tick();
  EXPECT_TRUE(_implementation.effects.empty());
  EXPECT_TRUE(_implementation.effect_loops.empty());
}
