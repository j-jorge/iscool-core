...
{
  const std::vector<int> values(iscool::random::distinct_random(5, 10, 20));

  EXPECT_EQ(5, values.size());
  EXPECT_EQ(5, std::unordered_set<int>(values.begin(), values.end()).size());
}

TEST(iscool_random_generator, positive_all)
{
  const std::vector<int> values(iscool::random::distinct_random(100, 0, 99));

  EXPECT_EQ(100, values.size());
  EXPECT_EQ(100, std::unordered_set<int>(values.begin(), values.end()).size());
}
...
{
  const std::vector<int> values(iscool::random::distinct_random(31, -10, 20));

  EXPECT_EQ(31, values.size());
  EXPECT_EQ(31, std::unordered_set<int>(values.begin(), values.end()).size());
}
