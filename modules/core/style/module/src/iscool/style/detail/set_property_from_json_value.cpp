...
namespace iscool
{
    namespace style
    {
        namespace detail
        {
            static std::vector< std::string > split_key_path
            ( const std::string & key );
            
            static std::vector< declaration > follow_declarations
            ( const declaration& style,
              const std::vector< std::string >& keys );
            
            static void insert_declarations
            ( std::vector< declaration >& declarations,
              const std::vector< std::string >& keys );
            
            static void set_property_value
            ( declaration& style, const std::string& key,
...
( const declaration& style, const std::vector< std::string >& keys )
{
    ...
    for ( std::size_t i( 0 ); i != count; ++i )
        result.emplace_back
            ( result.back().get_declaration_or_empty( keys[ i ] ) );
...
void iscool::style::detail::set_property_value
( declaration& style, const std::string& key, const Json::Value& property )
{
    if( iscool::json::is_of_type< float >( property ) )
        ...
        style.set_declaration
            ( key, get_merged_json_style_property( property ) );
    else
    {
        ic_causeless_log
