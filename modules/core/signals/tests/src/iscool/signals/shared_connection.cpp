     ...
     initially_connected_from_connected_source)
{
  ...
  iscool::signals::shared_connection c(signal.connect(
      []() -> void
      {
      }));
...
{
  iscool::signals::void_signal signal;
  iscool::signals::connection origin(signal.connect(
      []() -> void
      {
      }));
...
TEST(iscool_signals_shared_connection, not_equal_different_signals_connections)
{
  iscool::signals::void_signal first_signal;
  iscool::signals::shared_connection c1(first_signal.connect(
      []() -> void
      {
      }));

  iscool::signals::void_signal second_signal;
  iscool::signals::shared_connection c2(second_signal.connect(
      []() -> void
      {
      }));
...
TEST(iscool_signals_shared_connection, leaving_scope_disconnects)
{
  ...
  {
    iscool::signals::shared_connection c1(signal.connect(
        []() -> void
        {
        }));
