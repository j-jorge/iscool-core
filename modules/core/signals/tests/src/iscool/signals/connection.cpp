...
TEST(iscool_signals_connection, disconnect_copy)
{
  ...
  const iscool::signals::connection connection(signal.connect(
      []() -> void
      {
      }));
  iscool::signals::connection copy(connection);
...
TEST(iscool_signals_connection, different_from_different_callbacks)
{
  ...
  const iscool::signals::connection connection_1(signal.connect(
      []() -> void
      {
      }));
  const iscool::signals::connection connection_2(signal.connect(
      []() -> void
      {
      }));
