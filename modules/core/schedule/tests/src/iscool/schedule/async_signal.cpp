...
TEST(iscool_schedule_async_signal, connection)
{
  ...
  bool called(false);
  iscool_schedule_async_signal_macro_void_test test;
  iscool::signals::connection c(test.connect_to_test(
      [&called]() -> void
      {
        called = true;
      }));
...
IMPLEMENT_SIGNAL(iscool_schedule_async_signal_macro_arguments_test, test,
                 _signal);

void iscool_schedule_async_signal_macro_arguments_test::trigger(int i,
                                                                std::string s)
{
  _signal(i, s);
}
