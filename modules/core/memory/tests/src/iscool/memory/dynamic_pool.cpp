  ...
  namespace memory
  {
    namespace test
    {
      ...
      typedef iscool::memory::dynamic_pool<
          int, iscool::memory::test::empty_type_traits>
          int_pool;
    }
  }
}
...
namespace iscool
{
  namespace memory
  {
    namespace test
    {
      ...
      {
      public:
        void configure(std::size_t i, std::size_t p,
...
{
  if ((self.parent < self.dead->size()) && !(*self.dead)[self.parent])
    return false;
...
TEST(iscool_memory_dynamic_pool, destruction_order)
{
  ...
  iscool::memory::dynamic_pool<iscool::memory::test::destruction_order,
  ...
  for (std::size_t i(0); i != pool_size; ++i)
    {
      ...
      if (parent < pool_size)
        EXPECT_LT(std::find(begin, end, parent), std::find(begin, end, i));
      else
