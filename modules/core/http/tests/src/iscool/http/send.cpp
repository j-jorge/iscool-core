...
send_mockup::get( const std::string& url )
{
    auto on_result
...
iscool::signals::shared_connection_set send_mockup::post
( const std::string& url, const std::vector<std::string>& headers,
  const std::string& body )
{
    auto on_result
...
void
send_mockup::dispatch_response( int code, const std::string& body )
{
...
TEST( iscool_http_send_test, get_result )
{
    ...
    EXPECT_EQ
        ( iscool::http::request::type::get, mockup._requests[ 0 ].get_type() );
...
TEST( iscool_http_send_test, one_hundred_get_requests )
{
    ...
    for ( std::size_t i( 0 ); i != request_count; ++i )
        connections.insert
            ( mockup.get( boost::lexical_cast< std::string >( i ) ) );
...
TEST( iscool_http_send_test, post_result )
{
    ...
    ASSERT_EQ( 1ull, mockup._requests.size() );
    EXPECT_EQ
        ( iscool::http::request::type::post, mockup._requests[ 0 ].get_type() );
    EXPECT_EQ( url, mockup._requests[ 0 ].get_url() );
...
TEST( iscool_http_send_test, send_in_response )
{
    ...
    auto first_result
        ( [ &connections, &first_result_value, second_result, second_error ]
    ...
    requests[ 0 ].get_response_handler()
        ( iscool::http::response( 200, { '1' } ) );
    EXPECT_EQ( "1", first_result_value );
    
    EXPECT_EQ( 2ull, requests.size() );
    requests[ 1 ].get_response_handler()
        ( iscool::http::response( 200, { '2' } ) );
