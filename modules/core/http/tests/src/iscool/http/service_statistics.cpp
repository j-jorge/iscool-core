...
class iscool_http_service_statistics_test:
    public ::testing::Test
{
public:
...
{
    auto on_result( []( std::vector< char > result ) -> void {} );
    auto on_error( []( std::vector< char > error ) -> void {} );
    
    iscool::http::get( "http://www.example.org/", on_result, on_error );
}

void
iscool_http_service_statistics_test::dispatch_response( int code )
{
...
TEST_F( iscool_http_service_statistics_test, reset )
{
    ...
    EXPECT_PRED_FORMAT2
        (::testing::FloatLE, iscool::http::get_service_quality(), 1 );
