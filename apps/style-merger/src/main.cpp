...
options build_options(const boost::program_options::variables_map& arguments)
{
  ...
  result.search_path = arguments["search-path"].as<std::vector<std::string>>();

  return result;
}

void show_help(std::ostream& out, const std::string& program_name,
               const boost::program_options::options_description& options)
{
  out << "Usage:\n"
      << program_name << " [ --help ] --root=directory --search-path=directory"
      << " style-file…\n\n"
      << options << '\n';
}
...
boost::optional<options> parse_arguments(int argc, char** argv)
{
  ...
  options.add_options()("help", "Display this message and exit.")(
      "root",
      boost::program_options::value<std::string>()->value_name("directory"),
